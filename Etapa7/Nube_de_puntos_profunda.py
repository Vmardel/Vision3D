...
import matplotlib.pyplot as plt #muestra de graficos
import open3d as o3d #trabajo con puntos 3D
...
def resize_imagenes(img_izq, img_der, escala=1.0): #(al final esto no influye mucho puesto que redimensionamos las imagenes al principio del trabajo)
...
'''
estas funciones sirven para mejorar el contraste de las imagenes y para ver como varian los colores dentro de un margen seleccionado.
'''
...
'''
Con esta funicon ajustamos la  disparidad que calculamos.
'''
...
def main():
    ...
    mapa = calcular_disparidad(gris_izq, gris_der, tamaño_ventana=11, disparidad_max=128, usar_subpixeles=True)

    out_dir = "data"
    os.makedirs(out_dir, exist_ok=True)

    guardar_mapa_disparidad(mapa, "data/disparidad_profunda.png")
    ...
    generar_nube_puntos(mapa, np.array(img_izq), "data/nube_profunda.ply")
