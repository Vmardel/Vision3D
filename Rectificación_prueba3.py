...
def construir_homografias(K, R, t):
    ...
    ref = np.array([1, 0, 0]) if abs(np.dot(z, [1, 0, 0])) < 0.9 else np.array([0, 1, 0])
    x = np.cross(ref, z)
    x /= np.linalg.norm(x)
    y = np.cross(z, x)
...
def rectificacion_hartley(img_izq, img_der, F, pts1, pts2, out_dir="output"):
    ...
    os.makedirs(out_dir, exist_ok=True)
    guardar_imagen(os.path.join(out_dir, "rect_hartley_izq1.png"), rect1)
    guardar_imagen(os.path.join(out_dir, "rect_hartley_der1.png"), rect2)
